...
const getImage = (req, res) => {
  try {
    ...
    const pathFile = path.resolve(__dirname, "../uploads/users/" + image);

    fs.stat(pathFile, (exists) => {
        if(exists){
          console.log('exists: ', exists);
          return res.json({ image: pathFile })
        } else {
          return res.status(500).json({ error: 'There is no image' });
        }
    });
